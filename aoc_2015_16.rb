...
sues = []
sue_map = {}
A.split("\n").map{|a| a.split(" ")}.each do |a,b,c,d,e,f,g,h|
  sues << {c => d.to_i, e => f.to_i, g => h.to_i}
end
...
def possible_sue?(sue, sue_map)
  sue_map.each do |l,w|
    if sue.has_key?(l) && sue[l] != w
      return false
    end  
  end
  true
...
p sues.each_with_index.detect {|sue,i| possible_sue?(sue, sue_map)}.last+1

# 2

def real_sue?(sue, sue_map)
  sue_map.each do |l,w|
    if ["cats:", "trees:"].include?(l)
      if sue.has_key?(l) && sue[l] <= w
        return false
      end  
    elsif ["pomeranians:", "goldfish:"].include?(l)     
      if sue.has_key?(l) && sue[l] >= w
        return false
      end  
    elsif sue.has_key?(l) && sue[l] != w
      return false
    end  
  end
  true
...
p sues.each_with_index.detect {|sue,i| real_sue?(sue, sue_map)}.last+1
