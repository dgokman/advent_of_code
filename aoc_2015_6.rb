A = "toggle 461,550 through 564,900
...
lights = Array.new(1000){Array.new(10000){0}}

A.split("\n").map {|a| a.split(" ")}.each do |dir|
  if dir.length == 4
    start_y, start_x = dir[1].split(",").map(&:to_i)
    end_y, end_x = dir[3].split(",").map(&:to_i)
    for y in start_y..end_y
      for x in start_x..end_x
        lights[y][x] == 0 ? lights[y][x] = 1 : lights[y][x] = 0
      end
    end
  else
    start_y, start_x = dir[2].split(",").map(&:to_i)
    end_y, end_x = dir[4].split(",").map(&:to_i)
    for y in start_y..end_y
      for x in start_x..end_x
        dir[1] == "on" ? lights[y][x] = 1 : lights[y][x] = 0
      end
    end
  end
end

p lights.flatten.count(1)
...
A.split("\n").map {|a| a.split(" ")}.each do |dir|
  if dir.length == 4
    ...
    for y in start_y..end_y
      for x in start_x..end_x
        dir[1] == "on" ? lights[y][x] += 1 : (lights[y][x] -= 1 if lights[y][x] > 0)
      end
    end
  end
end
